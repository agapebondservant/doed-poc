...
class OcrProcessor:
    ...
    def export_to_markdown(self, input_file: str, output_dir: str) -> str:
        """Export specified input_file as markdown files to the output_dir directory."""
        
        conv_result = self.doc_converter.convert(input_file)

        os.makedirs(output_dir, exist_ok=True)
    
        with open(f"{output_dir}/{Path(input_file).stem}", "w") as f:

            exported = f.name
            
            print(f"Writing file: {exported}...")
